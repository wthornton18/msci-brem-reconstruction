        ...
        with uproot.open(self.filename) as file:
            ...
            for i in tqdm(range(self.N)):
                if nElectronTracks[i] < 1:  # Look for events with Electron Tracks
                    nET.append(i)
                elif ElectronTrack_TYPE[i,0] !=3: # only accept Track TYPE of 3 (which is mostly in index 0 of ElectronTrack_Type array for a given event)
                    nET.append(i)
            nET = np.array(nET)
            ...
            nCI_even = []  # nCI = no Cluster Info, i.e. remove events with no cluster info
