...
class ML_models:
    ...
    def optimise_model2(self,X_train,y_train,X_test,y_test, ratio):
        ...
        for train, test in tqdm(folds, total=len(folds)):
            ...
            ).set_params(**params).fit(dtrain[0],dtrain[1], eval_set= [dval], verbose = 0)
        ...
        try:
            ...
            title = 'ROC Curve'
        train_tpr_upper, train_tpr_mean, train_tpr_lower, train_auc = tp_rates('train',results)
...
def tp_fp_rate(model, train_data, val_data, test_data ):
    ...
    return (lr_fp_rates_train, lr_tp_rates_train) , (lr_fp_rates_val, lr_tp_rates_val) , (lr_fp_rates_test, lr_tp_rates_test), (train_auc,val_auc,test_auc)

def plot_roc_curve(training_rates, validation_rates, test_rates,train_val_test_auc, ratio):
...
if __name__ == '__main__':
    ...
    (X_train,y_train), (X_val,y_val), (X_test_t,y_test_t) = DP.train_validate_test_split(mixed_data_groups, validation_ratio = 0.001,test_ratio = 0.001,seed = 42)
